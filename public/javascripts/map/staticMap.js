/*Code for making the google map */
function initMap(json, wardCurlat, wardCurlong) {
	...
	var mapArray = [];
	//get each crime from json
	for (var l = 0; l < json.length; l++) {
		if (json[l].police.length > 0) {
			for (var z = 0; z < json[l].police.length; z++) {
				var tempAtt = [];
				if(json[l].police[z].outcome_status = null){
					json[l].police[z].outcome_status = none;
				}
				
				tempAtt.push(json[l].police[z].location.latitude, json[l].police[z].location.longitude, json[l].police[z].category, json[l].police[z].month,json[l].police[z].outcome_status,json[l].police[z].location.street.name);
				mapArray.push(tempAtt);
				console.log(json[l].police.length)
			}
		}
	}
	//for each of the crimes add a marker and randomise the location by a tiny bit otherwise all of the markers are stuck in one place
	for (var r = 0; r < mapArray.length; r++) {
		var fixeCat = mapArray[r][2].replace("-", " ");
		...
		//create a marker with the image and its content
		addMarker({
			latz: newLat,
			longz: newLng,
			...
			icon: '../images/map/'+mapArray[r][2]+'.png',
			content: '<h3>' + fixeCat.toUpperCase() + '</h3>'+'<p> Date:'+mapArray[r][3]+'<p>'+'<p>OutCome:'+mapArray[r][4]+'<p>'+'<p> Location:'+mapArray[r][5]+'<p>'
		});
	}
