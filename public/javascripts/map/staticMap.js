...
function initMap(json, wardCurlat, wardCurlong) {
	...
	for (var l = 0; l < json.length; l++) {
		if (json[l].police.length > 0) {
			for (var z = 0; z < json[l].police.length; z++) {
				var tempAtt = [];
				tempAtt.push(json[l].police[z].location.latitude, json[l].police[z].location.longitude, json[l].police[z].category, json[l].police[z].month);
	...
	for (var r = 0; r < mapArray.length; r++) {
		...
		addMarker({
			...
			content: '<h3>' + mapArray[r][2] + '</h3>'+'<p>'+mapArray[r][3]+'<p>'
		});
	}
